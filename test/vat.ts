...
describe("vat", async function () {
  ...
  before(async () => {
    ...
    vat = await simpleDeployL2("vat", {
      ward: _admin,
    }, hre);

    await starknet.devnet.dump("/home/nulven/dump.pkl");
    await sleep(5000);
  });
  ...
  beforeEach(async () => {
    await starknet.devnet.load("/home/nulven/dump.pkl");
  });
  ...
  it("test init", async () => {
    ...
    expect((await vat.call("ilks", { i: ILK })).ilk.rate).to.deep.equal(SplitUint.fromUint(RAY).res);
  });
  ...
  describe("flux", async function () {
    it("test flux self other", async () => {
      ...
      expect((await vat.call("gem", { i: ILK, u: _user1 })).gem).to.equal(SplitUint.fromUint(100n * WAD).res);
      expect((await vat.call("gem", { i: ILK, u: _user2 })).gem).to.equal(SplitUint.fromUint(0).res);
      ...
      expect((await vat.call("gem", { i: ILK, u: _user1 })).gem).to.equal(SplitUint.fromUint(0).res);
      expect((await vat.call("gem", { i: ILK, u: _user2 })).gem).to.equal(SplitUint.fromUint(100n * WAD).res);
    });
    ...
    it("test flux other self no permission", async () => {
      await admin.invoke(vat, "slip", { ilk: ILK, user: _user1, wad: SplitUint.fromUint(100n * WAD).res });
  ...
  describe("fold", async function () {
    ...
    it("test fold", async () => {
      ...
      expect((await vat.call("ilks", { i: ILK })).ilk.Art).to.deep.equal(SplitUint.fromUint(100n * WAD));
      ...
      expect((await vat.call("ilks", { i: ILK })).ilk.Art).to.deep.equal(SplitUint.fromUint(100n * WAD));
      expect((await vat.call("ilks", { i: ILK })).ilk.rate).to.deep.equal(SplitUint.fromUint((11n * RAY)/10n));
      expect((await vat.call("dai", { u: TEST_ADDRESS })).dai).to.deep.equal(SplitUint.fromUint(10));
    ...
    it("test fold negative", async () => {
      ...
      expect((await vat.call("ilks", { i: ILK })).ilk.Art).to.deep.equal(SplitUint.fromUint(100n * WAD));
      expect((await vat.call("ilks", { i: ILK })).ilk.rate).to.deep.equal(SplitUint.fromUint((11n * RAY)/10n));
      expect((await vat.call("dai", { u: TEST_ADDRESS })).dai).to.deep.equal(SplitUint.fromUint(10n * RAD));
      expect((await vat.call("debt")).debt).to.deep.equal(SplitUint.fromUint(110n * RAD));
      ...
      expect((await vat.call("ilks", { i: ILK })).ilk.Art).to.deep.equal(SplitUint.fromUint(100n * WAD));
      expect((await vat.call("ilks", { i: ILK })).ilk.rate).to.deep.equal(SplitUint.fromUint((21n * RAY)/20n));
      expect((await vat.call("dai", { u: TEST_ADDRESS })).dai).to.deep.equal(SplitUint.fromUint(5n * RAD));
